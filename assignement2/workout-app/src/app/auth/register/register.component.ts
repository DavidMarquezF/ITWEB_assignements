...
export class RegisterComponent implements OnInit {
  ...
  register(): void {
    if(this.form.invalid){
      this.snackBar.open('Error upon registration. Check if the form is correct.','Close',
        { duration: 5000 });
      return;
    }
    this._authService.register(this.form.value)
    .pipe(catchError(err => {
      this.snackBar.open(`Error upon registration. ${err.error.message}.`, 'Close',
        { duration: 5000 });
      return throwError(err);
    }),
    concatMap(() => this._authService.login({email: this.form.value.email,
      password : this.form.value.password})),
    catchError(err => {
      this.snackBar.open(`Registration unsuccessful! Error upon logging in. ${err.error.message}.`,
        'Close', { duration: 5000 });
      return throwError(err);
    }))
